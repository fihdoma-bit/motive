...
export const BudgetScreen: React.FC<BudgetScreenProps> = ({ currentUser, motives, updateBudget }) => {
  const [newBudget, setNewBudget] = React.useState(currentUser.monthly_budget.toString());

  const joinedMotives = useMemo(() => {
    return motives
      .filter(motive => motive.participants.some(p => p.id === currentUser.id))
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-24">
        <div className="max-w-md mx-auto space-y-6">
          <BudgetTracker budget={currentUser.monthly_budget} spent={totalSpent} />
              ...
              {joinedMotives.map(motive => (
                <div key={motive.id} className="bg-white p-4 rounded-2xl shadow-md flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <img src={motive.image_url} alt={motive.title} className="w-12 h-12 object-cover rounded-lg" />
                    <div>
