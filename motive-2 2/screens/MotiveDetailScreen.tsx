...
export const MotiveDetailScreen: React.FC<MotiveDetailScreenProps> = ({ motive, currentUser, onBack, onToggleJoin, onSendMessage, onViewProfile }) => {
  ...
  // Fetch initial chat messages and subscribe to realtime updates
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      async (payload) => {
        // Fetch the new message with author info
        const { data: newMsgData } = await supabase.from('profiles').select('*').eq('id', payload.new.user_id).single();
        if(newMsgData) {
            const newMessage: ChatMessage = {
                userId: newMsgData.id, name: newMsgData.name, avatar: newMsgData.avatar_url,
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto">
        <img src={motive.image} alt={motive.title} className="w-full h-52 object-cover" />
        ...
        <div className="p-4 max-w-md mx-auto space-y-6 pb-24">
          ...
          <div className="bg-white p-4 rounded-2xl shadow-md">
            <h3 className="font-bold text-lg text-gray-800 mb-3">Participants ({motive.participants.length}/{motive.maxParticipants})</h3>
              ...
              <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                {(chatMessages && chatMessages.length > 0) ? chatMessages.map((msg, index) => (
                  <div key={index} className={`flex items-end gap-2 ${msg.userId === currentUser.id ? 'justify-end' : 'justify-start'}`}>
                    {msg.userId !== currentUser.id && <img src={msg.avatar} className="w-8 h-8 rounded-full" />}
